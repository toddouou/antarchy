...
> **STATUS: planning complete + APPROVED (2026-06-06). Implementation NOT started.**
> **Branch to create:** `security-p0` off `beta-v1`.
> **Resume at:** §4-first-substep (Argon2id foundation), then §1 AoI fix. See "Execution order" +
> "Next concrete action" at the bottom.
...
Create branch `security-p0`, then start §4a: add `argon2`/`password-hash`/`subtle` to `Cargo.toml`
and implement `hash_pw_argon2` / `verify_pw` / `needs_rehash` in `src/auth.rs` + the
`HIVE_ARGON2_*` config helpers. Nothing implemented yet.
